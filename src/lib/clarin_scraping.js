...
let ln_rss_url = 'https://www.clarin.com/rss/lo-ultimo/'
...
const getNota = async (link ) => {
    ...
    let title = fetch.$('#title').text().trim().replace(/[^ -~]+/g, "").replace(/([\"\'])/g,'\\"');
    let dropline = fetch.$('.bajada').text().trim();
    ...
    if (!fs.existsSync(storage_path + md5(link) )) {
        fs.writeFile(storage_path + md5(link), title + "\n" + dropline + '\n' + body, function (err) {
            ...
            db_help.insert(link, md5(link), title, dropline, "CL", null)
            console.log("Adding Clarin " + link + " - " + md5(link));
        });
    }
};
